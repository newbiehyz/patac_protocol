...
class SimDataLoader {
 public:
  ...
  bool PopOutMea(ReplaySensorType& type, long long& ts);
 ...
 private:
  ...
  std::map<long long, Pose> _gt_pose;
  std::map<long long, std::vector<SemanticMea::Ptr>> _semantic_mea;
  std::map<long long, std::vector<KinematicMea::Ptr>> _kinematic_mea;
  std::map<long long, std::vector<ReplaySensorType>> _mea_seq;
  std::map<long long, std::vector<ReplaySensorType>>::iterator _mea_it;
  int _mea_id{0};
};
}  // namespace apa_slam
