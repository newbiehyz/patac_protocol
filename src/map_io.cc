...
bool MapIO::SaveMapData() {
  ...
  if (margin_data_list_.empty() && 
      slot_map_data_list_.empty() && 
...
// TODO
bool MapIO::LoadMapData(const std::string& map_file_path, const std::string& window_file_path) {
...
template<typename T>
bool MapIO::SerializeAndWrite(std::ofstream& file, const std::vector<T>& data_list) {
