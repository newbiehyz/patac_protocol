...
#include "interface/parking/serializer/dynamic_obstacles_serializer.h"
...
int DynamicObstaclesSerializer::Serialize(
...
int DynamicObstaclesSerializer::Deserialize(
  ...
  if (static_cast<std::uint32_t>(dynamic_obsts_data.num_obstacles) >
      DynamicObstaclesData::kMaxDynamicObstacleSize) {
